...
export default function Artwork(props: IArtworkProps) {
  const { beatId, artworkKey, playPauseStatus, onClick } = props;
  ...
  return (
    <div className={styles['image-container']}>
      <Image
        ...
        alt="album artwork"
        preview={{
          mask: <Image src={playPauseStatus === 'playing' ? pauseIcon : playIcon} preview={false} />,
          visible: false,
        }}
        ...
        // }}
        onClick={() => {
          if (playPauseStatus !== 'loading') onClick();
        }}
        className={styles.artwork}
        data-cy="beat-artwork"
      />
      {playPauseStatus === 'loading' && beatPlayingFromState?._id == beatId ? (
        <LoadingOutlined className={styles['playing-animation']} />
      ) : null}
      {playPauseStatus === 'playing' && beatPlayingFromState?._id == beatId ? (
        <div className={styles['playing-animation']}>
