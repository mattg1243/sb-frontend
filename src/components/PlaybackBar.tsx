...
export default function PlayBackBar(props: IPlayBackBarProps) {
  ...
  return (
    ...
    <Button style={{ position: 'fixed', bottom: '0', background: 'var(--primary)', borderColor: 'black'}} onClick={() => { setIsShownState(!isShownState)}}><CaretUpOutlined size={5} /></Button>
    <Drawer 
    placement='bottom'
    closable={true}
    onClose={() => { setIsShownState(false)}}  
    open={isShownState}
    mask={false}
    height='200px'
    style={{ 
